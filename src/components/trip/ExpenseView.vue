  ...
  <div class="space-y-8 animate-fade-in pb-20">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div 
        ...
        class="bg-white p-5 rounded-3xl border-2 shadow-sm transition-all group relative overflow-hidden cursor-pointer active:scale-95"
      ...
      >
        <div class="relative z-10">
          ...
          <div class="flex items-baseline gap-1">
            <span class="text-xs font-bold text-secondary-400">{{ preferredCurrency }}</span>
            <span class="text-2xl font-black text-secondary-900 font-mono">{{ formatAmount(stat.value) }}</span>
          </div>
          ...
          >
            <div v-if="expandedStat === stat.label" class="mt-4 pt-4 border-t border-secondary-50 space-y-2">
               <div v-for="(val, cur) in stat.breakdown" :key="cur" class="flex justify-between text-[10px] font-bold">
                 <span class="text-secondary-400 uppercase">{{ cur }}</span>
                 <span class="text-secondary-700 font-mono">{{ formatNumSimple(val) }}</span>
               </div>
            </div>
          </transition>
        </div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-center justify-between bg-white p-2 rounded-2xl border-2 border-secondary-50 shadow-sm sticky top-4 z-20 backdrop-blur-md bg-white/90">
      <div class="flex items-center gap-4 w-full sm:w-auto">
        <div class="flex bg-secondary-50 p-1 rounded-xl">
           <button 
             @click="viewMode = 'list'"
             class="px-4 py-1.5 rounded-lg text-xs font-black transition-all"
             :class="viewMode === 'list' ? 'bg-white text-secondary-900 shadow-sm' : 'text-secondary-400 hover:text-secondary-600'"
           >
             支出明細
           </button>
           <button 
             ...
             class="px-4 py-1.5 rounded-lg text-xs font-black transition-all"
             :class="viewMode === 'settlement' ? 'bg-white text-secondary-900 shadow-sm' : 'text-secondary-400 hover:text-secondary-600'"
           >
             結算總覽
           </button>
        </div>
        <div v-if="viewMode === 'list'" class="h-6 w-px bg-secondary-100 hidden sm:block"></div>
        <ExpenseFilter v-if="viewMode === 'list'" />
      </div>

      <button 
        @click="isModalOpen = true"
        class="w-full sm:w-auto px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl shadow-lg shadow-primary-200 font-black transition-all flex items-center justify-center gap-2 transform active:scale-95"
      >
    ...
    <div class="grid lg:grid-cols-3 gap-8 items-start">
      ...
      <div v-if="viewMode === 'list'" class="lg:col-span-1 bg-secondary-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-secondary-200">
      ...
      <div :class="viewMode === 'list' ? 'lg:col-span-2' : 'lg:col-span-3'" class="space-y-6">
        ...
        <template v-if="viewMode === 'list'">
          <div class="flex items-center justify-between mb-4 px-2">
            <h3 class="text-xl font-black text-secondary-900">
              {{ filterMode === 'mine' ? '我的支出' : '支出明細' }}
            </h3>
            <span class="text-[10px] font-black text-secondary-400 uppercase tracking-widest">{{ filteredExpenses.length }} 筆記錄</span>
          </div>

          <div class="space-y-10">
            <div v-for="(group, date) in groupedExpenses" :key="date" class="space-y-4">
               <div class="bg-secondary-50 px-4 py-1.5 rounded-full inline-block">
                 <h4 class="text-[10px] font-black text-secondary-500 uppercase tracking-widest">{{ date }}</h4>
               </div>
            ...
            <div v-if="filteredExpenses.length === 0" class="py-24 text-center bg-secondary-50 rounded-[2.5rem] border-4 border-dashed border-secondary-100">
               <div class="w-16 h-16 bg-secondary-100 rounded-full flex items-center justify-center mx-auto mb-6">
                 <ShoppingBagIcon class="w-8 h-8 text-secondary-300" />
               </div>
               <p class="text-lg font-black text-secondary-900">目前沒有相關支出</p>
               <p class="text-sm font-bold text-secondary-400 mt-2 italic">開始記帳來追蹤旅程預算吧！</p>
            </div>
          </div>
        </template>
